...
const DataChatPage = () => {
  ...
  const [activeVisualization, setActiveVisualization] = useState<ChatMessage['visualization'] | null>(null)
  const [isGeneratingChart, setIsGeneratingChart] = useState(false)
  const messagesEndRef = useRef<HTMLDivElement>(null)

  // Debug: log activeVisualization changes
  useEffect(() => {
    console.log('activeVisualization changed:', activeVisualization)
  }, [activeVisualization])
  ...
  const chatMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Auto-open visualization if detected
      if (data.visualization) {
        console.log('Setting active visualization:', data.visualization) // Debug log
        setActiveVisualization(data.visualization)
      }
  ...
  // Mutation for generating visualization HTML
  ...
  const handleRefreshVisualization = () => {
    if (activeVisualization?.endpoint && activeVisualization?.payload) {
      setIsGeneratingChart(true)
      chartMutation.mutate({
        endpoint: activeVisualization.endpoint,
        payload: activeVisualization.payload
      })
    }
  }

  const handleViewVisualization = (msgVisualization: ChatMessage['visualization']) => {
    if (msgVisualization) {
      setActiveVisualization(msgVisualization)

      // If HTML is not available, try to generate it
      if (!msgVisualization.html && msgVisualization.endpoint && msgVisualization.payload) {
        setIsGeneratingChart(true)
        chartMutation.mutate({
          endpoint: msgVisualization.endpoint,
          payload: msgVisualization.payload
        })
      }
    }
  }
  ...
  return (
    <div className="h-[calc(100vh-120px)] flex gap-0">
      ...
      <div className="flex-1 flex flex-col">
        ...
        {/* Chat Messages */}
        <div className="flex-1 bg-white rounded-lg shadow mx-4 overflow-y-auto p-6 space-y-6 mb-4">
          {messages.length === 0 && (
            <div className="text-center py-12 text-gray-500">
              ...
              <div className="mt-6 space-y-2 text-sm">
                ...
                <div className="flex flex-col gap-2 max-w-md mx-auto">
                  <button
                    onClick={() => setQuestion("Create a pie chart showing distribution by category")}
                    className="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded text-left text-sm"
                  >
                    "Create a pie chart showing distribution by category"
                  </button>
                  <button
                    onClick={() => setQuestion("Show a scatter plot of X vs Y")}
                    className="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded text-left text-sm"
                  >
                    "Show a scatter plot of X vs Y"
                  </button>
                  <button
                    onClick={() => setQuestion("What is the average value by region?")}
                    className="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded text-left text-sm"
                  >
                    "What is the average value by region?"
                  </button>
                </div>
              </div>
            </div>
            ...
            <div key={index} className="space-y-3">
              ...
              <div className="flex justify-start">
                <div className="bg-gray-100 px-4 py-3 rounded-lg max-w-[70%] space-y-3">
                  <p className="text-gray-900 whitespace-pre-wrap text-sm">{msg.answer}</p>
                  ...
                  {msg.visualization && (
                    <button
                      onClick={() => handleViewVisualization(msg.visualization)}
                      className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-700 hover:underline transition"
                    >
                      <BarChart3 size={16} />
                    ...
                    <div className="mt-3">
                      ...
                      {msg.result_type === 'series' && typeof msg.result === 'object' && (
                        <div className="overflow-x-auto">
                          <table className="min-w-full text-xs border">
                            <tbody>
                              {Object.entries(msg.result).slice(0, 10).map(([key, value]: [string, any]) => (
                                <tr key={key} className="border-b">
                                  <td className="px-2 py-1 font-medium bg-gray-50">{key}</td>
                      ...
                      {msg.result_type === 'dataframe' && Array.isArray(msg.result) && msg.result.length > 0 && (
                        <div className="overflow-x-auto">
                          <table className="min-w-full text-xs border">
                            ...
                            <tbody>
                              {msg.result.slice(0, 5).map((row: any, idx: number) => (
                      ...
                      {msg.result_type === 'list' && Array.isArray(msg.result) && (
                        <div className="space-y-1">
                          {msg.result.slice(0, 5).map((item: any, idx: number) => (
                            <div key={idx} className="px-2 py-1 bg-white rounded text-xs">
        ...
        {/* Input Form */}
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-4 mx-4 mb-4">
